...
import { DndFileComponent } from 'src/app/frontend/common/dnd-file/dnd-file.component';
...
export class UtilStatic {
  ...
  static toJSONObject(obj): any {
    return JSON.parse(JSON.stringify(obj, null, 2));
  }

  static getDateFromDateTimePickerString(selectedDate) {
  ...
  static getDateFromDatePicker(selectedDate): Date | null {
  ...
  static getRecipePicture(dndComponent: DndComponent) {
  ...
  static getCSVFromDnd(dndComponent: DndComponent) {
    if (dndComponent.files != null && dndComponent.files[0] != null) {
      return dndComponent.files[0];
    } else {
      return null;
  ...
  static getFileFromDnd(dndComponent: DndFileComponent) {
    if (dndComponent.files != null && dndComponent.files[0] != null) {
      return dndComponent.files[0];
    }
    return null;
  ...
  static sortInstructions(instructions) {
  ...
  static filterByNames(items: any, names: any): any {
    const nameSet = new Set(names.map(name => name.toLowerCase())); // Ignorer la casse
