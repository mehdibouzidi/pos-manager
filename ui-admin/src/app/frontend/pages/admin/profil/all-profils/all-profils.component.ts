...
import { ProfilCriteria } from 'src/app/backend/criteria/admin/profilcriteria';
...
@Component({
  selector: 'vex-all-profils',
  standalone: true,
  templateUrl: './all-profils.component.html',
  styleUrl: './all-profils.component.scss',
...
export class AllProfilsComponent {
//Back Variables
  criteria = new ProfilCriteria();
  globalPayload = new GlobalPayload<ProfilPayload>();
  ...
  ngOnInit(): void {
    this.dataSource = new MatTableDataSource<ProfilPayload>();
  ...
  findByCriteria() {
    ...
    this.service.findAllByCriteria(this.criteria)
      ...
      .subscribe({
        next: (response: GlobalPayload<ProfilPayload>) => {
        if (response != null) {
  ...
  selection = new SelectionModel<ProfilPayload>(true, []);

  @Input()
  columns: TableColumn<ProfilPayload>[] = [
    {
      label: 'Code',
      property: 'code',
      type: 'text',
  ...
  toggleColumnVisibility(column: TableColumn<ProfilPayload>, event: Event) {
  ...
  goToAdd(): void {
    ...
    goToEdit(element: ProfilPayload) {
      this.router.navigate(['./edit', element.id], {
        relativeTo: this.route,
      });
    }
  
    goToShow(element: ProfilPayload) {
