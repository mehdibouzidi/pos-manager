...
export class AddProfilComponent {
  generalForm: FormGroup;
    ...
    add(): void {
      ...
      this.profilPrivileges = this.profilPrivileges.filter((privilege) => privilege !== null);
    ...
    addPrivilege() {
      if (this.privilege && this.privilege.id) {
        ...
        this.dataSource.data = this.profilPrivileges;
        this.privileges = this.privileges.filter((elt) => elt.id != this.privilege.id);
    ...
    resetPrivilegeAutocomplete() {
      this.filteredPrivileges = this.privilegeControl.valueChanges.pipe(
        startWith(''),
        map((value) => (typeof value === 'string' ? value : value.name)),
        map((name) => (name ? this._filterPrivileges(name) : this.privileges.slice()))
      );
      this.privilege = null;
