...
export class EditUserComponent {
      ...
      next: (response: Array<ProfilPayload>) => {
        if (response != null) {
          ...
          this.profils = this.profils.filter(
            (elt) => !this.userProfils.some((userProfil) => userProfil.id == elt.id)
          );
  ...
  addProfil() {
    if (this.profil && this.profil.id) {
      ...
      this.dataSource.data = this.userProfils;
      this.profils = this.profils.filter((elt) => elt.id != this.profil.id);
  ...
  deleteProfil(element) {
    ...
    console.log(element);
    this.userProfils = this.userProfils.filter((elt) => elt.id != element.id);
    this.dataSource.data = this.userProfils;
  ...
  resetProfilAutocomplete() {
    this.filteredProfils = this.profilControl.valueChanges.pipe(
      startWith(''),
      map((value) => (typeof value === 'string' ? value : value.name)),
      map((name) => (name ? this._filterProfils(name) : this.profils.slice()))
    );
    this.profil = null;
