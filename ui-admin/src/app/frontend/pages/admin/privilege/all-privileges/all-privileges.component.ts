...
import { PrivilegeCriteria } from 'src/app/backend/criteria/admin/privilegecriteria';
...
@Component({
  selector: 'vex-all-privileges',
  standalone: true,
  templateUrl: './all-privileges.component.html',
  styleUrl: './all-privileges.component.scss',
...
//Back Variables
  criteria = new PrivilegeCriteria();
  globalPayload = new GlobalPayload<PrivilegePayload>();
  ...
  ngOnInit(): void {
    this.dataSource = new MatTableDataSource<PrivilegePayload>();
  ...
  findByCriteria() {
    ...
    this.service.findAllByCriteria(this.criteria)
      ...
      .subscribe({
        next: (response: GlobalPayload<PrivilegePayload>) => {
        if (response != null) {
  ...
  selection = new SelectionModel<PrivilegePayload>(true, []);

  @Input()
  columns: TableColumn<PrivilegePayload>[] = [
    {
      label: 'Code',
      property: 'code',
      type: 'text',
  ...
  toggleColumnVisibility(column: TableColumn<PrivilegePayload>, event: Event) {
  ...
  goToEdit(element: PrivilegePayload) {
    const dialogRef = this.dialog.open(EditPrivilegeComponent, {
      width: '950px',
      data: { element }
    });
